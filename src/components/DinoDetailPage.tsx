...
import { Dino, EffectKind } from '../game/types'
...
import { getEffectDuration } from '../lib/effects'
  ...
  return (
    <div className="flex-1 flex flex-col p-4 gap-6 overflow-y-auto relative bg-gradient-to-br from-stone-950 via-amber-950/20 to-stone-950">
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto space-y-4 lg:space-y-6">
        ...
        {/* Top row: name + class & spec (side by side on desktop, stacked on mobile) */}
                  ...
                  <div className="flex gap-2 px-2">
                    {dino.class && (
                      <div className="flex items-center justify-center gap-2 bg-amber-900/10 border border-amber-900/30 rounded-lg px-3 py-2 flex-1">
                        <MedallionIcon id={dino.class} type="class" size="lg" />
                    ...
                    {dino.spec && (
                      <div className="flex items-center justify-center gap-2 bg-amber-900/10 border border-amber-900/30 rounded-lg px-3 py-2 flex-1">
                        <MedallionIcon id={dino.spec} type="spec" size="lg" />
                        <span className="text-xs font-black text-amber-950">
        ...
        {/* Stat bonus allocator (full width when active) */}
        ...
        {/* Main layout.
            Desktop: abilities (left, 2/3) + statistics (right, 1/3) on one row,
                     experience bar full width below.
            Mobile:  statistics, then experience, then abilities (stacked). */}
        <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4 lg:gap-6 items-start">
          ...
          <motion.div
            ...
            className="w-full order-3 lg:order-1 lg:col-span-2"
          >
            <PremiumCard variant="frame">
              <div className="space-y-4">
                ...
                {/* Slot Grid — compact; content scales via container queries */}
                              ...
                              <div className="flex flex-wrap gap-2">
                                {ability.effects.map((effect) => (
                                  <button
                                    key={effect}
                                    type="button"
                                    onClick={() => setEffectModal(effect as EffectKind)}
                                    className="flex items-center gap-1.5 bg-amber-900/12 hover:bg-amber-900/25 transition-colors border border-amber-900/30 rounded px-2.5 py-1 cursor-pointer"
                                    title={`${getEffectNameTR(effect)} — detaylar`}
                                  >
          ...
          {/* Statistics — each stat its own premium Hearthstone card */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      <AnimatePresence>
        ...
        {effectModal && (
          <EffectInfoModal
            effect={{ type: effectModal, duration: getEffectDuration(effectModal, 1) }}
            isOpen={!!effectModal}
            onClose={() => setEffectModal(null)}
          />
        )}
      </AnimatePresence>
    </div>
