 ...
 */
export default function AbilityTypeIcon({ kind, size = 'sm', className = '' }: AbilityTypeIconProps) {
  const [failed, setFailed] = useState(false)
  const url = getAbilityTypeIconUrl(kind)

  if (failed) {
    return (
      <span className={`${sizeMap[size]} flex items-center justify-center flex-shrink-0 ${className}`}>
        {emojiFallback[kind]}
      </span>
    )
  }
